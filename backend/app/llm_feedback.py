...
def _build_evidence_section(
    ...
    # Slide text + similarity for SLIDE_READING, CONTENT_COVERAGE
    if slide_text and similarity > 0.0:
        ...
        parts.append(
            ...
            "Do NOT observe CONTENT_COVERAGE."
        )
...
def _validate_observations(
    ...
    for item in items[:1]:  # Max 1 observation per slide (CONTENT_COVERAGE only)
        if not isinstance(item, dict):
            continue
        ...
        validated.append(
            ObservationItem(
                type=ObservationType.content_coverage,
                detail=detail,
                evidence=evidence,
            )
        )
